...
def main():
    ...
    actual_transcripts = nemo_model.transcribe(audio_filepath, batch_size=args.batch_size)

    print("Pytorch Transcripts :")
    print(actual_transcripts)
    print("========================================")

    flat_transcripts = [h.text for h in actual_transcripts]
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for test_batch in tqdm(temporary_datalayer, desc="ONNX Transcribing"):
            ...
            # Process hypothesis (map char/subword token ids to text)
            decoded = nemo_model.decoding.decode_hypothesis(hypotheses)

            # Luôn ép về list
            if isinstance(decoded, list):
                texts = [h.text for h in decoded]
            else:
                texts = [decoded.text]

            all_hypothesis.extend(texts)
    ...
    if args.log:
        for pt_transcript, onnx_transcript in zip(flat_transcripts, all_hypothesis):
    ...
    pt_onnx_cer = word_error_rate(all_hypothesis, flat_transcripts, use_cer=True)
